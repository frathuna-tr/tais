...
export default function Profil() {
  ...
  // Profil yüklenince avatar'ı göster
  useEffect(() => {
    if (profil && (profil as any).avatar_url) {
  ...
  return (
    <>
      ...
      {yukleniyor && (
        <div style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'#f2f2f0',zIndex:9998,fontFamily:"'DM Sans',sans-serif",fontSize:'13px',color:'rgba(0,0,0,0.4)'}}>
          Yükleniyor...
        </div>
      )}
      ...
      <div style={{display:'grid',gridTemplateColumns:'220px 1fr',height:'100vh',visibility: yukleniyor ? 'hidden' : 'visible'}}>
          ...
          <div style={{marginTop:'auto',borderTop:'1px solid rgba(255,255,255,0.08)',padding:'14px 16px'}}>
            <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
              <div style={{width:'34px',height:'34px',background:'#00c48c',borderRadius:'2px',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,color:'#0a0a0a',fontFamily:"'Pixelify Sans',monospace",overflow:'hidden'}}>
                {avatarUrl ? <img src={avatarUrl} style={{width:'100%',height:'100%',objectFit:'cover'}}/> : `${profil?.ad?.[0] ?? ''}${profil?.soyad?.[0] ?? ''}`}
              </div>
              <div>
                <div style={{fontSize:'12px',color:'rgba(255,255,255,0.8)',fontWeight:500}}>{profil?.ad} {profil?.soyad}</div>
                <div style={{fontSize:'9px',color:'rgba(255,255,255,0.3)',marginTop:'2px'}}>{profil?.unvan} · {profil?.universite?.split(' ').slice(-1)[0] ?? ''}</div>
              </div>
            </div>
          </div>
        </aside>
            ...
            <div style={{background:'#fff',border:'1px solid rgba(0,0,0,0.07)'}}>
              ...
              <div style={{padding:'0 24px 20px',display:'flex',alignItems:'flex-end',gap:'16px'}}>
                ...
                <div style={{flex:1,paddingTop:'8px',minWidth:0}}>
                  <div style={{fontFamily:"'Pixelify Sans',monospace",fontSize:'18px',fontWeight:700,color:'#0a0a0a',marginBottom:'3px'}}>{profil?.ad} {profil?.soyad}</div>
                  <div style={{fontSize:'12px',color:'rgba(0,0,0,0.45)',marginBottom:'2px'}}>{profil?.unvan} · {profil?.bolum}</div>
                  <div style={{fontSize:'12px',color:'#00c48c',fontWeight:600,marginBottom:'8px'}}>{profil?.universite}</div>
