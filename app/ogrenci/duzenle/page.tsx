...
// ── CSS component dışında → render'da yeniden oluşmaz ─────────────
const css = `
  * { margin:0; padding:0; box-sizing:border-box; cursor:none!important; }
  ...
  .nav-row { display:flex; align-items:center; gap:10px; padding:9px 18px; font-size:13px; cursor:pointer; border-left:2px solid transparent; transition:color 0.15s; }
  .nav-row:hover { background:rgba(255,255,255,0.05); }
  .nav-row.active { color:#00c48c; border-left-color:#00c48c; background:rgba(0,196,140,0.08); font-weight:600; }
...
export default function OgrenciProfilDuzenle() {
  ...
  const [ad, setAd] = useState('Zeynep')
  const [soyad, setSoyad] = useState('Arslan')
  const [eposta, setEposta] = useState('z.arslan@itu.edu.tr')
  const [telefon, setTelefon] = useState('')
  const [sehir, setSehir] = useState('İstanbul')
  const [bio, setBio] = useState('Yapay zeka ve sağlık teknolojilerinin kesişiminde çalışıyorum.')
  const [ilgi, setIlgi] = useState('Derin Öğrenme, Tıbbi Görüntüleme, PyTorch')
  const [program, setProgram] = useState('Doktora Öğrencisi')
  const [uni, setUni] = useState('İstanbul Teknik Üniversitesi')
  const [fakulte, setFakulte] = useState('Fen Bilimleri Enstitüsü')
  const [bolum, setBolum] = useState('Bilgisayar Mühendisliği')
  const [tez, setTez] = useState('Derin Öğrenme Yöntemleriyle Tıbbi Görüntü Segmentasyonu')
  const [danisman, setDanisman] = useState('Prof. Dr. Ahmet Yılmaz')
  ...
  const navItems = [
    ...
    { label:'Akademik Ağ', href:'/ag', icon:<svg viewBox="0 0 12 12" width="14" height="14"><circle cx="2" cy="6" r="1.5" fill="#06b6d4"/><circle cx="6" cy="2" r="1.5" fill="#06b6d4"/><circle cx="10" cy="6" r="1.5" fill="#06b6d4"/><circle cx="6" cy="10" r="1.5" fill="#06b6d4"/><line x1="2" y1="6" x2="6" y2="2" stroke="#06b6d4" strokeWidth="1"/><line x1="6" y1="2" x2="10" y2="6" stroke="#06b6d4" strokeWidth="1"/><line x1="2" y1="6" x2="6" y2="10" stroke="#06b6d4" strokeWidth="1"/><line x1="6" y1="10" x2="10" y2="6" stroke="#06b6d4" strokeWidth="1"/></svg> },
    { label:'CV', href:'/cv', icon:<svg viewBox="0 0 12 12" fill="#fbbf24" width="14" height="14"><rect x="2" y="1" width="8" height="10"/><rect x="4" y="0" width="4" height="2"/><rect x="4" y="3" width="4" height="1"/><rect x="4" y="5" width="5" height="1"/></svg> },
    { label:'Mesajlar', href:'/mesajlar', icon:<svg viewBox="0 0 12 12" fill="rgba(255,255,255,0.45)" width="14" height="14"><rect x="1" y="2" width="10" height="7"/><rect x="1" y="9" width="2" height="2"/><rect x="2" y="8" width="8" height="1"/></svg> },
    { label:'Profilim', href:'/ogrenci', icon:<svg viewBox="0 0 12 12" fill="#94a3b8" width="14" height="14"><rect x="4" y="1" width="4" height="4"/><rect x="1" y="8" width="10" height="3"/><rect x="2" y="7" width="2" height="1"/><rect x="8" y="7" width="2" height="1"/></svg> },
    { label:'Profil Düzenle', href:'/ogrenci/duzenle', icon:<svg viewBox="0 0 12 12" fill="#00c48c" width="14" height="14"><rect x="1" y="9" width="10" height="1"/><path d="M2 7l5-5 2 2-5 5z" fill="#00c48c"/></svg> },
  ]
      ...
      <div style={{display:'grid',gridTemplateColumns:'220px 1fr',height:'100vh'}}>
          ...
          <div style={{padding:'10px 0',flex:1}}>
            {navItems.map((item, i) => (
              <div key={i} onClick={() => router.push(item.href)}
                className={'nav-row' + (item.href === '/ogrenci/duzenle' ? ' active' : '')}
                style={{color: item.href === '/ogrenci/duzenle' ? '#00c48c' : 'rgba(255,255,255,0.55)'}}>
          ...
          <div style={{borderTop:'1px solid rgba(255,255,255,0.08)',padding:'14px 16px'}}>
            <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
              ...
              <div>
                ...
                <div style={{fontSize:'9px',color:'rgba(255,255,255,0.3)',marginTop:'2px'}}>{program}</div>
              </div>
            </div>
          </div>
        </aside>
        ...
        <div style={{display:'flex',flexDirection:'column',height:'100vh',overflow:'hidden'}}>
          ...
          <div style={{background:'#fff',borderBottom:'1px solid rgba(0,0,0,0.07)',padding:'0 32px',height:'52px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
            <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
              <span onClick={() => router.push('/ogrenci')} style={{display:'flex',alignItems:'center',gap:'5px',cursor:'pointer',color:'rgba(0,0,0,0.4)',fontSize:'12px',fontWeight:500}}>
            ...
            <div style={{display:'flex',gap:'8px'}}>
              <button className="tbtn-ol" onClick={() => router.push('/ogrenci')}>İptal</button>
                ...
                {sekme==='genel' && (
                  <div style={{display:'flex',flexDirection:'column',gap:'16px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'16px',padding:'16px',background:'#f8f9fa',border:'1px solid rgba(0,0,0,0.06)'}}>
                      <div style={{width:'60px',height:'60px',background:'#00c48c',borderRadius:'4px',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Pixelify Sans',monospace",fontSize:'20px',fontWeight:700,color:'#0a0a0a',flexShrink:0}}>
                        {ad[0]}{soyad[0]}
                      </div>
                      <div>
                        ...
                        <label style={{fontSize:'11px',fontWeight:600,color:'#00c48c',border:'1px solid #00c48c',padding:'4px 10px',cursor:'pointer'}}>
                          Fotoğraf Yükle <input type="file" accept="image/*" style={{display:'none'}}/>
                        </label>
                      </div>
                    </div>
                    <div className="grid2">
                    ...
                    <div><label className="f-lbl">İlgi Alanları</label><textarea className="f-ta" rows={2} value={ilgi} onChange={e=>setIlgi(e.target.value)} placeholder="Virgülle ayırın..."/></div>
                  </div>
                )}

                {sekme==='akademik' && (
                  <div style={{display:'flex',flexDirection:'column',gap:'16px'}}>
                    <div><label className="f-lbl">Program</label>
                      <select className="f-sel" value={program} onChange={e=>setProgram(e.target.value)}>
                        <option>Lisans Öğrencisi</option>
                        <option>Yüksek Lisans Öğrencisi</option>
                        <option>Doktora Öğrencisi</option>
                      </select>
                    </div>
                    <div><label className="f-lbl">Üniversite</label><input className="f-inp" value={uni} onChange={e=>setUni(e.target.value)}/></div>
                    <div className="grid2">
                      <div><label className="f-lbl">Fakülte / Enstitü</label><input className="f-inp" value={fakulte} onChange={e=>setFakulte(e.target.value)}/></div>
                      <div><label className="f-lbl">Bölüm</label><input className="f-inp" value={bolum} onChange={e=>setBolum(e.target.value)}/></div>
                    </div>
                    <div>
                      <label className="f-lbl">Öğrenci No</label>
                      <input className="f-inp" value="2021***42" disabled/>
                      <span style={{fontSize:'10px',color:'rgba(0,0,0,0.3)',marginTop:'4px',display:'block'}}>Değiştirilemez.</span>
                    </div>
                    <div style={{borderTop:'1px solid rgba(0,0,0,0.07)',paddingTop:'16px',display:'flex',flexDirection:'column',gap:'12px'}}>
                      <div style={{fontSize:'10px',fontWeight:700,color:'rgba(0,0,0,0.35)',textTransform:'uppercase',letterSpacing:'0.5px'}}>Tez / Araştırma</div>
                      <div><label className="f-lbl">Tez Konusu</label><textarea className="f-ta" rows={2} value={tez} onChange={e=>setTez(e.target.value)}/></div>
                      <div><label className="f-lbl">Danışman</label><input className="f-inp" value={danisman} onChange={e=>setDanisman(e.target.value)} placeholder="Prof. Dr. ..."/></div>
                    </div>
                    <div className="info-box">💡 Yayınlar ve projeler ilgili sayfalardan yönetilir.</div>
                  </div>
                )}
